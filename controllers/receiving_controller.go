...
func (c *ReceivingController) GetInbound(rw http.ResponseWriter, r *http.Request) (error, int) {
	vars := mux.Vars(r)
	inbound_id, err := strconv.Atoi(vars["id"])
	if err != nil {
		return err, http.StatusBadRequest
	}
	...
	inbound, err = dao.Get_Inbound(inbound_id)
	if err == sql.ErrNoRows {
		//no need to log 404?
		return err, http.StatusNotFound
	} else if err != nil {
